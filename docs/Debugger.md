# Debugger
Debugger is a [native binary](./Native.md) named MonoDebugger.


## reference implementation for configuring the debugger when it launches
``` ts
...
Notes:
- `cwd` is the Unity project path
- for settings, we only provide `projectAssembliesOnly`, `stepOverPropertiesAndOperators`, both of them are true by default
- `transportId` is `dotrush-${process.pid}`