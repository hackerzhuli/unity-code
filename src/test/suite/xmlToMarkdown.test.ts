...
The right operand of the addition.
        ...
        const result = xmlToMarkdown(input);
        assert.ok(result.includes('**Parameter `left`:** The left operand of the addition.'));
        assert.ok(result.includes('**Parameter `right`:** The right operand of the addition.'));
    });
...
The sum of two integers.
        ...
        const expected = `**Returns:** The sum of two integers.`;
        assert.strictEqual(xmlToMarkdown(input), expected);
    });
...
Thrown when one parameter is greater than MaxValue and the other is greater than 0.
        ...
        const result = xmlToMarkdown(input);
        assert.ok(result.includes('**Exception `System.OverflowException`:** Thrown when one parameter is greater than MaxValue'));
    });
...
for this instance.
        ...
        const result = xmlToMarkdown(input);
        assert.ok(result.includes('**Value:** The `Label` property represents a label'));
    });
...
greater than 0.
        ...
        assert.ok(result.includes('**Returns:** The sum of two integers.'));
        assert.ok(result.includes('**Parameter `left`:** The left operand of the addition.'));
        assert.ok(result.includes('**Parameter `right`:** The right operand of the addition.'));
        assert.ok(result.includes('**Exception `System.OverflowException`:**'));
...
This tag will apply to the primary constructor parameter.
        ...
        const result = xmlToMarkdown(input);
        assert.ok(result.includes('**Summary:**'));
        assert.ok(result.includes('This is an example of a positional record.'));
        assert.ok(result.includes('**Remarks:**'));
        assert.ok(result.includes('**Parameter `FirstName`:** This tag will apply to the primary constructor parameter.'));
        assert.ok(result.includes('**Parameter `LastName`:** This tag will apply to the primary constructor parameter.'));
    });
});
